...
ClusterMarker.prototype._clusterMarker=function($clusterGroupIndexes){
	function $newClusterMarker($location, $icon, $title){
		return new GMarker($location, {icon:$icon, title:$title, draggable: true});
	}
	...
	this._eventListeners.push(GEvent.addListener($clusterMarker, 'dragend', function(m){
                Ext.MessageBox.show({
                    title:'Save Changes?',
                    msg: 'Are you sure you want to change the geo coordinates of all the ' + $clusteredMarkers.length + ' items managed by this marker?',
                    ...
                    fn: function(button) { 
                        if (button == 'yes') {
                            ...
                            for (var i=0; i < $clusteredMarkers.length; i++) {
                                ...
                                items.push(marker.gmap.lastMarkers[marker.map_index].item);
                            }
                           ...
                           $clusteredMarkers[0].gmap.onMapMove();
                        }
                    },
                    animEl: 'mb4',
...
ClusterMarker.prototype.clusterMarkerClickByIndex=function($index){
	var $marker=this._clusterMarkers[$index];
	this._map.setCenter($marker.getLatLng(), this._map.getBoundsZoomLevel($marker.clusterGroupBounds));
};
	...
	if(this._activeMarkersChanged || $forceFullRefresh){
		...
		for(i=this._clusterMarkers.length-1; i>=0; i--){
			this._map.addOverlay(this._clusterMarkers[i]);
                        this._clusterMarkers[i].cluster_index = i;
		}
