...
def add_variants(e,i,current_workspace, workspace_id):
    ...
    for namevariant in oldvariants.keys():
        for typevariant in oldvariants[namevariant].keys():
            if oldvariants[namevariant][typevariant]['auto_generated'] == False:
                #verificare se esiste gia'
                if not (namevariant in newvariants):
                    #se non esiste creo
                    data = []
    ...
    for namevariant in oldvariants.keys():
        for typevariant in oldvariants[namevariant].keys():
            if  oldvariants[namevariant][typevariant]['auto_generated'] == True:
                ...
                if not (namevariant in newvariants.keys()):
                    data.append(('name', namevariant))
                    data.append(('auto_generated', oldvariants[namevariant][typevariant]['auto_generated']))
                    data.append(('media_type', oldvariants[namevariant][typevariant]['media_type']))
                    data.append(('caption', oldvariants[namevariant][typevariant]['caption']))
                    for d in oldvariants[namevariant][typevariant]['sources']:
