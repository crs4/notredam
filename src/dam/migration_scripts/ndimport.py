...
#    logging.debug('----------------oldvariant %s'%oldvariants)
 ...
 #   logging.debug('----------------newvariants %s'%newvariants)              
    ...
    for dict_variant in oldvariants['variants']:

        data = []
        data.append(('workspace_id', workspace_id))
        data.append(('name', dict_variant['name']))
        data.append(('caption', dict_variant['caption']))
        data.append(('media_type', dict_variant['media_type']))
        data.append(('auto_generated', dict_variant['auto_generated']))

        #verificare se esiste gia'
        if not (dict_variant in newvariants['variants']):
            #se non esiste creo
            i._variants_new(data)
        else:
            i._variants_edit(dict_variant['id'],data)
...
def search_original_shortname(current_item):
    ...
    """
    print "#########################inizio search original shortname" 
    try:
...
def get_data_for_upload(current_variant,id_workspace,file_name,filepath,shortname,variant_ws):
    ...
    print "get data for upload finished"
    return param
...
def upload_variants(current_item,id_workspace,variant_ws,id_item,file_name,shortname_original, upload_orig = False):
    ...
    for variant in custom_listfiles(current_item):
        ...
        if extension[1:] != 'json' and shortname != shortname_original:
            ...
            file = open(current_variant)     
            print current_variant           
            param['Filedata'] = file
            try:
                print "chiamataaaaaaaaaaaaaaaaaaaaaaaaaa"
                v_url = 'http://127.0.0.1:8000/api/item/%s/upload/' % id_item
                param = urllib.urlencode(param)
                u = urllib2.urlopen(v_url, param)
    ...
    for itemdir in custom_listdirs(current_workspace):
        ...
        if os.path.isdir(current_item) and itemdir != 'watermarking':#exclude folder watermarking
            ...
            if len(paramitem['workspaces']) == 1:#NOT shared object
                ...
                print "id_orig_itemToid_new_item %s" %id_orig_itemToid_new_item

                #variant's upload
                print "upload varianti"
                upload_variants(current_item,paramitem['workspace_id'],variant_ws,resp_new_item['id'],shortname_original,shortname_original,True)
