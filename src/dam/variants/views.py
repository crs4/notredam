...
#    media_type = request.POST['media_type']
    ...
    resp = {'variants':[]}
    for variant in vas:
        resp['variants'].append({'pk':variant.pk,  'name': variant.name, 'auto_generated': variant.auto_generated, 'is_global': variant.workspace is None, 'media_type': [m.name for m in variant.media_type.all()] })
...
#            info_list.append({'caption': 'File Size', 'value': '%s' % comp.format_filesize()})
            ...
            logger.debug("get_variants: %s" % str(ex))
