...
def get_variants_menu_list(request):
    ...
    vas = Variant.objects.filter(Q(workspace = workspace) | Q(workspace__isnull = True)).exclude(name='original').exclude(name='thumbnail').order_by('name').values_list('name', flat=True)  
